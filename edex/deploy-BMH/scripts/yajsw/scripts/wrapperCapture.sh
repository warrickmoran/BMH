...
# Sep 10, 2014  3456     bkowal      BMH version of the script.
#
#####################################################################
# NOTE: Script must be located at /awips2/bmh/bin/yajsw/scripts for it to work
...
checkDir() {
   ...
   if [ ! -d "$dir" ]; then
      mkdir -p $dir
      if [ ! -d "$dir" ]; then
         message="Unable to create CommsManager capture data directory\n$dir"
   ...
   if [ ! -w "$dir" ]; then
      message="Do not have write permissions to CommsManager capture data directory\n$dir"
      echo -e "Capture failed: $message"
      exit 1
   fi
}
